function allsubj_results = create_results_struct_sigtestupdate(is_within_subject, summed_mcpa, parsed_input, all_sets, num_subj, num_sets, num_feature, num_cond, final_dimensions)
...
allsubj_results.feature_space = parsed_input.Results.feature_space;
...
for cond_id = 1:num_cond % now create place holders for decoding accuracies 
    ...
    if is_within_subject
         allsubj_results.accuracy(cond_id).subjXsession = nan(num_subj,size(summed_mcpa.patterns, session_dim));
    end
end
